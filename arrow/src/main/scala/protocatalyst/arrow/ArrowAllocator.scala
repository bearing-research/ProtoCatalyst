...
/** Memory allocator management for Arrow operations.
  *
  * Arrow uses off-heap memory for vectors, which requires explicit allocation and deallocation.
  * This object provides utilities for safe allocation management.
  *
  * Usage:
  * {{{
...
object ArrowAllocator:
  ...
  /** Create a child allocator from a parent allocator.
    *
    * Child allocators share the parent's memory limit but allow tracking memory usage separately.
    *
    * @param parent
    *   Parent allocator
    * @param name
    *   Name for the child allocator (for debugging)
    * @param initialReservation
    *   Initial memory reservation (0 = no reservation)
    */
  def createChild(
