...
- **End-to-end execution**: with a 2-line `ScalaReflection` patch (below; upstream on master since
  2026-08-23, still needed for released 4.1.x), the derived encoder round-trips real values through
  Spark's **unmodified** codegen ser/deser from a Scala 3 process (`ExecutionWallSpec`) — and drives a
  real typed `Dataset` query through a stock `SparkSession` (`SparkSessionWallSpec`).
...
1. **Call it from `ExpressionEncoder.apply[T]()` on the Scala 3 build** — `deriveAgnosticEncoder[T]`
   ...
   is unchanged.
2. ~~**De-reflect the `ScalaReflection` object** so it loads on Scala 3.~~ **Already upstream, as of
   2026-08-23.** The two lines (`val universe` → `lazy val`; `encodeFieldNameToIdentifier` →
   `NameTransformer.encode`) landed on master as
   [SPARK-57548](https://github.com/apache/spark/pull/57303), and the matching `SparkSession.builder()`
   wall as [SPARK-58169](https://github.com/apache/spark/pull/57302) — both reached independently of
   this project, which is a useful corroboration of the diagnosis. Spark **4.1.x still carries both**,
   so module `spark-reflection-patch` remains what lets this repo drive released 4.1.2 from Scala 3
   (`ExecutionWallSpec`, `SparkSessionWallSpec`). Targeting master, **this step is done** and the ask is
   items 1 and 3. (`findConstructor`'s rarely-hit companion-`apply` fallback is still reflective
   upstream; de-reflected here and proven by `FindConstructorFallbackSpec` — an edge-case follow-up
   someone may still file. See [UPSTREAM_SESSION_BUILDER.md](docs/scala3-encoder/UPSTREAM_SESSION_BUILDER.md).)
