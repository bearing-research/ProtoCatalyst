 ...
 *
 * Uses Scala's Dynamic trait to intercept field access and return
 * typed Column references. The macro ensures type safety at compile time.
 *
 * Usage:
 * {{{
...
final class FieldSelector[A] private[dsl] (
  ...
  /**
   * Dynamic field access - returns a Column for the field.
   * Type safety is enforced by the macro in TypedOps.
   */
