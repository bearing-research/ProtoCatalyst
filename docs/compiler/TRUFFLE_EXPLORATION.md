...
(58 parity tests as of this writing). Operators fully compose via a table-catalog execution model — WHERE/SELECT/GROUP BY/HAVING/ORDER BY all run over joins, with residual join conditions.*
...
- **Operators:** filter, projection, global + grouped aggregate (null-aware), `ORDER BY`/`LIMIT`/
  `DISTINCT`, inner + outer (LEFT/RIGHT/FULL) equi-joins, nested-loop / cross / non-equi joins, and the full window-function family
...
- **Composition:** a table-catalog execution model (`run(Map[String, Batch])`) lets operators fully
  ...
  correct for outer joins. Aliased scans qualify their fields (`n.col`) and references resolve
  qualifier-aware (exact `q.name`, else unqualified suffix), so a join's *shared* column names
  (`n.regionkey` vs `r.regionkey`) disambiguate — matching the executor's `resolveColumn`. The
  single-table Truffle-PE fast path is kept; the over-join layer is Scala row-at-a-time
  (`RowEval`/`Row*Query`). Parity-checked on the TPC-H Q3/Q5/Q10 shape.
...
(`ROWS`/`RANGE BETWEEN`) and the Layer-5 `SparkPlan` front-end that any real Spark integration would
require.
...
3. **Stateful operators.** Global + grouped aggregate (HashAggregate accumulator model), Sort, Limit,
   ...
   *exactly* (column order + values). **Outer joins** are in too — LEFT/RIGHT/FULL with null padding
   ...
   BETWEEN`) and join conditions beyond equi-keys. (Filters/projects/aggregates now compose with
   joins via the catalog execution model — see the Results summary.)
