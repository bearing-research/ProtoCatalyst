...
| `LocalTimeEncoder`                  | `java.time.LocalTime`     | `TimeType()` (default precision 6)| `given ProtoEncoder[LocalTime]`    | `TimeType(6)`              | ✅ encoder added in Spark 4.1; **but** `SerializerBuildHelper` still throws `UNSUPPORTED_TIME_TYPE` as of 4.1.2 — Spark hasn't lit up the full write path yet. ProtoCatalyst's `UnsafeRowSerializer` writes nanos-since-midnight today; cross-validate once Spark closes the gap. |
