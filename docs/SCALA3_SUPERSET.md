...
|---|---|---|---|
| `java.util.UUID` | `StringType` | reject | reject (D3) — could `TransformingEncoder(StringEncoder, uuid↔string)` |
| `java.time.OffsetDateTime` | `TimestampType` | reject | reject — could wrap `↔ Instant` |
| `java.time.ZonedDateTime` | `TimestampType` | reject | reject — could wrap `↔ Instant` |
| `java.util.Date` | `TimestampType` | reject | reject — could wrap `↔ Timestamp` |
...
— they'd be just as derivable in Scala 2.13. They have no Spark oracle and, because execution is
blocked (§2.1), no behavioral test either; the faithful path is `TransformingEncoder`-wrapping,
deferred. Listed here for completeness of "beyond Spark."
...
- **Superset** (Spark can't): **self-consistency** only — assert the bridge produces the expected
  `AgnosticEncoder` shape, or rejects with the right message. No Spark golden can exist, and
  end-to-end execution against stock Spark is blocked by the residual `scala.reflect.runtime`
  (§2.1), so structural self-consistency is the achievable bar.
...
| UUID / OffsetDateTime / ZonedDateTime / java.util.Date / LocalTime | reject | reject today; `TransformingEncoder` is the faithful path (deferred) | none (no oracle) |
