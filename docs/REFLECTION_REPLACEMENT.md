...
`encoderFor` returns `AgnosticEncoder[E]` — a pure ADT — and **everything downstream is already
reflection-free** (`ExpressionEncoder` → Catalyst expressions → codegen). ~16 other files merely
*thread a `TypeTag`* into `encoderFor`; on Scala 3 those become `[T]`-with-a-derived-given signature
swaps (forced — `TypeTag` doesn't exist).
...
1. **Structural parity (primary):** `bridge(ProtoEncoder.derived[T])` vs a golden
   `ScalaReflection.encoderFor[T]` (golden generated by a Scala 2.13 helper — the cross-compile
   fixture pattern we already use for Arrow). Compare node-by-node (`==`; `ClassTag` by `runtimeClass`).
2. **Schema parity:** the lowered `dataType` equal to Spark's.
3. **End-to-end sanity:** `ExpressionEncoder(ours)` drives `spark.createDataset(...).collect()` and
   round-trips — a behavioral oracle, not an integration claim.

> Java reflection in deserializer `NewInstance`/`Invoke` is *not* what we replace (works on Scala 3);
> it only appears in the end-to-end sanity check.
...
- **M0 — Bridge spike:** decide §3; lower `ProtoEncoder.derived[Person]` (flat case class) to
  `AgnosticEncoder`; structural-parity vs a 2.13 golden; end-to-end `Dataset` round-trip. *The engine
  already exists, so M0 is the bridge + oracle, not re-derivation.*
