...
/** User-Defined Type (UDT) for ProtoCatalyst.
  *
  * A UDT allows custom types to be stored in Spark DataFrames by defining how to
  * serialize/deserialize to a standard SQL type.
  *
  ...
  *
  * @tparam UserType
  *   The user-facing type (must be a reference type, can be null)
  */
trait ProtoUDT[UserType >: Null]:

  /** The underlying SQL type used for storage. Common patterns:
  ...
  /** Serialize a user object to a SQL datum.
    ...
    *
    * @param obj
    *   The user object to serialize (may be null)
    * @return
    *   SQL datum compatible with sqlType
    */
  ...
  /** Deserialize a SQL datum back to the user type.
    *
    * @param datum
    *   The SQL datum (compatible with sqlType)
    * @return
    *   The user object
    */
  def deserialize(datum: Any): UserType

  /** The Class of the user type. Used for runtime type checking and ClassTag creation.
    */
  def userClass: Class[UserType]

  /** ClassTag for the user type. Derived from userClass.
    */
  final def classTag: ClassTag[UserType] = ClassTag(userClass)

  /** Whether this UDT supports null values. Default is true (reference types can be null).
    */
  def nullable: Boolean = true

  /** The ProtoType representation of this UDT. Wraps the sqlType with UDT metadata.
    */
  final def protoType: ProtoType = ProtoType.UDTType(getClass.getName, sqlType)

  /** Optional: Friendly type name for display purposes. Defaults to the simple class name of the
    * user type.
    */
  def typeName: String = userClass.getSimpleName

object ProtoUDT:
  /** Registry of UDT instances by class name. Allows looking up UDTs at runtime for
    * deserialization.
    */
  private val registry = scala.collection.mutable.Map[String, ProtoUDT[?]]()

  /** Register a UDT instance for later lookup. Called automatically when creating UDT encoders.
    */
