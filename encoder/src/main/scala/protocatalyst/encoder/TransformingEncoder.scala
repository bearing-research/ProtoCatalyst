...
import protocatalyst.encoder.codec.{BinaryCodec, JavaSerializationCodec, KryoSerializationCodec, KryoCodecImpl, ForySerializationCodec, ForyCodecImpl}
...
/**
 * Encoder that uses a codec for types that cannot be encoded natively.
 * The value is serialized to bytes and stored as BinaryType.
 *
 * This is the fallback for types that can't be represented using standard encoders,
 * such as third-party library types without a ProtoUDT definition.
 *
 ...
 *
 * @param clsTag ClassTag for the external type
 * @param codecProvider Factory for creating codec instances (lazy for serialization)
 */
...
object TransformingEncoder:
   ...
   *
   * @param codecProvider Factory function that creates the codec instance
   */
  ...
  /**
   * Create a TransformingEncoder using Java serialization.
   * No external dependencies required, but slower than Kryo or Fory.
   *
   * Example:
   * {{{
  ...
  /**
   * Create a TransformingEncoder using Kryo serialization.
  ...
  /**
   * Create a TransformingEncoder using Apache Fory (formerly Fury) serialization.
   * Up to 170x faster than Java serialization with Scala 3 native support.
   * Requires fory-scala_3 dependency.
   *
   * Example:
   * {{{
   * case class ThirdPartyType(data: String)
