...
/** Type-specialized row serializer using compile-time inline expansion.
  *
  * This serializer eliminates runtime type dispatch by generating specialized serialization code
...
object InlineRowSerializer:
  ...
  /** Derive an inline-specialized serializer at compile time.
    *
    * The serialization logic is generated inline at the call site, with type-specialized code paths
    * for each field.
    */
  ...
  inline def serializeFieldsImpl[Types <: Tuple](
    ...
    inline erasedValue[Types] match
      ...
      case _: (Map[k, v] *: ts) =>
        ...
        result(idx) =
          conv.toInternal(map, ProtoType.MapType(getProtoType[k], getProtoType[v], true))
        serializeFieldsImpl[ts](product, result, idx + 1, conv)
  ...
  inline def deserializeFieldsImpl[Types <: Tuple](
    ...
    inline erasedValue[Types] match
      ...
      case _: (Map[k, v] *: ts) =>
        val rawMap =
          conv.fromInternal(row(idx), ProtoType.MapType(getProtoType[k], getProtoType[v], true))
  ...
  private inline def deserializeSeqElementsToList[T](
      seq: Seq[?],
      conv: InternalTypeConverter
  ): List[T] =
  ...
  private inline def deserializeMapElements[K, V](
      map: Map[?, ?],
      conv: InternalTypeConverter
  ): Map[K, V] =
    map.map { case (k, v) =>
  ...
  private inline def deserializeAnyField[T](value: Any, conv: InternalTypeConverter): T =
    if value == null then null.asInstanceOf[T]
      ...
      inline erasedValue[T] match
        ...
        case _: Map[k, v] =>
          val rawMap =
            conv.fromInternal(value, ProtoType.MapType(getProtoType[k], getProtoType[v], true))
