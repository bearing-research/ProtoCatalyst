...
object ParityTester {
  ...
  /** Test parity for a SQL statement and its ProtoCatalyst JSON representation.
    ...
    * @param sql
    *   The SQL statement
    * @param protoCatalystJson
    *   JSON string containing the ProtoCatalyst CompiledArtifact
    * @param spark
    *   The SparkSession to use for parsing SQL
    * @return
    *   ParityResult with comparison details
    */
  def testParity(sql: String, protoCatalystJson: String)(implicit
      spark: SparkSession
  ): ParityResult = {
  ...
  def testParityFromBytes(sql: String, artifactBytes: Array[Byte])(implicit
      spark: SparkSession
  ): ParityResult = {
    val sparkPlan = spark.sessionState.sqlParser.parsePlan(sql)
  ...
  def testParityFromPlanJson(sql: String, planJson: Json)(implicit
      spark: SparkSession
  ): ParityResult = {
    val sparkPlan = spark.sessionState.sqlParser.parsePlan(sql)
