...
// `org.apache.spark.sql.catalyst.ScalaReflection` (sql-api module) with exactly
// TWO lines changed, to prove the Scala-3 execution wall is removable.
//
// The wall (reproduced in encoder-spark WallReproSpec): from a Scala 3 process,
// `Invoke.encodedFunctionName` -> `ScalaReflection.encodeFieldNameToIdentifier`
// touches this object, whose `<clinit>` eagerly forces
...
object ScalaReflection extends ScalaReflection {
  lazy val universe: scala.reflect.runtime.universe.type = scala.reflect.runtime.universe // PROTOCATALYST PATCH (1/2): was `val`
  ...
  def encodeFieldNameToIdentifier(fieldName: String): String = {
    // PROTOCATALYST PATCH (2/2): was `TermName(fieldName).encodedName.toString` (runtime universe).
    // NameTransformer.encode is the same name-mangling, from scala-library, no runtime reflection.
