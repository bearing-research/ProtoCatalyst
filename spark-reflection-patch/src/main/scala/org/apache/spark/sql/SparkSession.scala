...
// =============================================================================
// PROTOCATALYST DEMONSTRATOR PATCH — verbatim copy of Apache Spark 4.1.2's
// `org.apache.spark.sql.SparkSession` (sql-api module) with ONE method body
// changed, to prove the *second* Scala-3 execution wall is removable.
//
// The first wall is `ScalaReflection`'s eager runtime universe (see the copy of
// that file in this module). This one is upstream of it and hits sooner: from a
// Scala 3 process, `SparkSession.builder()` -> `DEFAULT_COMPANION` ->
// `lookupCompanion` -> `scala.reflect.runtime.currentMirror` fails to
// initialize, so a Scala 3 user cannot obtain a SparkSession at all. Because
// `DEFAULT_COMPANION` wraps the lookup in `Try` and the failure is `NonFatal`,
// the real cause is swallowed and reported as the misleading
// "Cannot find a SparkSession implementation on the Classpath."
//
// The change (search "PROTOCATALYST PATCH"): `lookupCompanion` reads the Scala
// object's static `MODULE$` field via Java reflection instead of building a
// Scala runtime mirror. Same fix shape as `ScalaReflection.findConstructor`'s
// companion-`apply` fallback, and equally behaviour-preserving on 2.13.
//
// Proven by `encoderSpark/testOnly *SparkSessionWallSpec` — with this file on
// the classpath, stock `SparkSession.builder()` works from Scala 3.
// =============================================================================
...
object SparkSession extends SparkSessionCompanion {
  ...
  // ===== PROTOCATALYST PATCH (SparkSession wall) =====================================
  // Upstream body (Spark 4.1.2):
  //
  ...
  //
  // `scala.reflect.runtime.currentMirror` cannot initialize on the Scala 3 stdlib
  // (scala/scala3#25896: `FatalError: class Array does not have a member apply`), so on Scala 3
  // this throws for BOTH candidates. `DEFAULT_COMPANION` catches that with `Try` — the error is
  // `NonFatal` — and rethrows it as "Cannot find a SparkSession implementation on the Classpath.",
  // which sends the user hunting a phantom classpath problem. Net effect: `SparkSession.builder()`
  // is unusable from Scala 3, independent of anything encoder-related.
  //
  // All the reflection is doing is fetching a Scala object instance. On the JVM that object is a
  // static `MODULE$` field on the `<name>$` class, so a direct field read is equivalent — and it
  // is what `SparkClassUtils.classForName` already implies we can do. Same fix shape as
  // `ScalaReflection.findConstructor`'s companion-`apply` fallback.
  private[this] def lookupCompanion(name: String): SparkSessionCompanion = {
    // Force the implementation class itself to load first, so a genuinely absent implementation
    // still fails here (as upstream does) rather than at the MODULE$ read.
    SparkClassUtils.classForName(name)
    val companionClass = SparkClassUtils.classForName(name + "$")
    companionClass.getField("MODULE$").get(null).asInstanceOf[SparkSessionCompanion]
  }
  // ===== END PROTOCATALYST PATCH ====================================================
