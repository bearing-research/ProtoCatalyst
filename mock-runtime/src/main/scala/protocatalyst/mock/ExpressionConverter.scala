...
import protocatalyst.plan.{SortOrder as ProtoSortOrder, SortDirection as ProtoSortDirection, NullOrdering as ProtoNullOrdering}
...
object ExpressionConverter:
  ...
  def toMock(expr: ProtoExpr): MockExpression =
    ...
    expr match
      ...
      case ScalarSubquery(plan) =>
        ...
        val resultType = mockPlan.output.headOption.map(_.dataType).getOrElse(MockDataType.StringType)
      ...
      case Lead(input, offset, default) => ME.Lead(toMock(input), toMock(offset), default.map(toMock))
    ...
    expr match
      ...
      case ME.UnresolvedAttribute(nameParts) =>
        nameParts match
          ...
          case Seq(qualifier, name) => PE.ColumnRef(name, Some(qualifier), ProtoType.UnresolvedType("unresolved"), true)
          case parts => PE.ColumnRef(parts.last, Some(parts.init.mkString(".")), ProtoType.UnresolvedType("unresolved"), true)
      ...
      case ME.InSubquery(value, plan) => PE.InSubquery(fromMock(value), PlanConverter.fromMock(plan))
      ...
      case ME.Lead(input, offset, default) => PE.Lead(fromMock(input), fromMock(offset), default.map(fromMock))
      case ME.Lag(input, offset, default) => PE.Lag(fromMock(input), fromMock(offset), default.map(fromMock))
  ...
  private def convertMockLiteral(value: Any, dataType: MockDataType): ProtoExpr =
    ...
    if value == null then
      Literal(NullValue(TypeConverter.fromMock(dataType)))
    else dataType match
...
object TypeConverter:
  ...
  def fromMock(mdt: MockDataType): ProtoType =
    ...
    mdt match
      ...
      case MDT.StructType(fields) => PT.StructType(fields.map(f =>
